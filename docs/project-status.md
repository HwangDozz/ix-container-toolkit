...
> 说明：本文保留大量 Iluvatar 联调时期的状态描述。当前仓库主线已经转向 `profile 渲染 + 统一运行视图`，因此涉及静态 `RuntimeClass` / `DaemonSet` 和 `pkg/config` 主配置角色的旧表述，应以较新的 `docs/generic-framework-p0-baseline.md`、`docs/generic-profile-schema.md` 与代码实现为准。
...
### 3.1 `accelerator-container-hook`（核心注入器）
...
9. 写入 `<rootfs>/etc/ld.so.conf.d/accelerator-toolkit.conf`，使动态链接器找到驱动库
...
### 3.2 `accelerator-container-runtime`（runtime shim）
...
3. 若存在，将 `accelerator-container-hook` 前插到 `hooks.prestart` 数组头部，写回 `config.json`
...
### 3.3 `accelerator-installer`（节点安装器）
...
- installer 已支持读取 generic profile
- installer 会把活动 profile 复制到宿主机 `/etc/accelerator-toolkit/profiles/active.yaml`
- installer / runtime / hook 入口现在通过统一运行视图装配 `profile + config`
- `pkg/config` 仍存在，但当前角色是旧 JSON 兼容层
...
**实现文件**：`cmd/accelerator-installer/main.go`
...
accelerator-container-runtime:
  ...
  3. 将 accelerator-container-hook 写入 hooks.prestart[0]
  4. exec runc create --bundle <path> ...
          │
          ▼
runc 在容器进程启动前执行 prestart hook:
  调用 accelerator-container-hook（stdin = OCI state JSON）
          │
          ▼
...
3. 调用 `accelerator-container-runtime create --bundle <bundle>`，验证 hook 注入
4. 以 OCI state JSON 为 stdin 调用 `accelerator-container-hook`，验证注入结果
...
| `ld.so.conf.d/accelerator-toolkit.conf` | PASS — 文件正确写入，包含两条库路径 |
...
目前已具备 `make docker-build-multiarch` / `make docker-build-prebuilt-multiarch` 入口，但仍建议添加 GitHub Actions 或 Tekton Pipeline 实现：
- 自动构建并推送 `linux/amd64,linux/arm64` 多架构镜像 manifest
- 自动准备 `bin/<os>-<arch>/` 预构建产物（若走 prebuilt 流程）
- 自动运行单元测试

#### 8. accelerator-installer 单元测试

`cmd/accelerator-installer/main.go` 的 `copyBinaries`、`writeConfig`、`patchContainerd` 函数目前无测试覆盖，可通过传入临时目录路径实现 mock 文件系统测试。

#### 9. 卸载流程

当前 `make undeploy` 只删除 DaemonSet 和 RBAC，未清理：
- 宿主机上的二进制（`/usr/local/bin/ix-container-*`）
- 宿主机上的配置（`/etc/accelerator-toolkit/`）
